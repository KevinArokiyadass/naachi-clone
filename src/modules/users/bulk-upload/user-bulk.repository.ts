...
import { ExistingUserLookupMaps } from './user-bulk-upload.types';
...
export class UserBulkRepository {
  ...
  async findExistingByUniqueKeys(
    ...
    const [emailRows, userNameRows, phoneRows] = await Promise.all([
      emails.length
        ? this.dbServices.users.find(
            { email: { $in: emails }, isDeleted: false },
            { userId: 1, email: 1 },
          )
        : Promise.resolve([]),
      userNames.length
        ? this.dbServices.users.find(
            { userName: { $in: userNames }, isDeleted: false },
            { userId: 1, userName: 1 },
          )
        : Promise.resolve([]),
      phoneNumbers.length
    ...
    return {
      ...
      byUserName: new Map(
        userNameRows.filter((row) => row?.userName).map((row) => [row.userName, row]),
      ),
      byPhoneNumber: new Map(
        phoneRows.filter((row) => row?.phoneNumber).map((row) => [row.phoneNumber, row]),
      ),
    };
  }
}
