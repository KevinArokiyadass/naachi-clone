...
const HEADER_ALIAS_MAP: Record<string, string> = {
  ...
  'email id': 'email',
  status: 'status',
  'select permission': 'permissionGroupName',
...
@Injectable()
export class AdminUserBulkParser {
  ...
  parse(file: Express.Multer.File): ParsedAdminUserUploadRow[] {
    ...
    try {
      ...
      throw new BadRequestException({
        code: BulkUploadErrorCode.INVALID_FILE_FORMAT,
  ...
  private validateRequiredColumns(row: Record<string, unknown>): void {
    const normalizedColumns = new Set(
      Object.keys(row).map((key) => key.trim()),
    );
